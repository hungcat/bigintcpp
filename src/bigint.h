...
public:
	...
	BigInt(std::string str, uint_long radix = 10);  // in Radix Conversion
...
BigInt::BigInt(std::string str, uint_long radix) : BigInt() {
	...
	if (r) {
		operator+=(BigInt((int_long)std::stoul(str.substr(0, r), 0, radix)));
	}

	for (uint_long i = 0; i < q; ++i) {
		operator*=(base);
		operator+=(BigInt((int_long)std::stoul(
